...
const socket = io("https://chess-game-socket.onrender.com");


const ChessBoard = () => {
  const [board, setBoard] = useState<any[][] | null>(null);
  const [selectedCell, setSelectedCell] = useState<{ rowIndex: number; colIndex: number; piece: string | null } | null>(null);
  const [isWhiteTurn, setIsWhiteTurn] = useState(true);
  const [gameStatus, setGameStatus] = useState('ongoing');
  const [winner, setWinner] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [capturable, setCapturable] = useState<{ [key: string]: boolean }>({});
  const [loadingMove , setLoadingMove] = useState<Boolean | null>(null);
  const [playCode , setPlayCode] = useState(12345);


  const {code} = useParams<{ code: string }>();

  useEffect(() => {
    setPlayCode(Number(code));
  } , [code]);
  ...
  const updateBoard = () => {
    setBoard(
      chess
        ...
        .map(row =>
          row.map(piece =>
            piece ? (piece.color === 'w' ? piece.type.toUpperCase() : piece.type) : null
          )
        )
    );
  };
  ...
  const getPieceComponent = (piece: string, rowIndex: number, colIndex: number) => {
    ...
    return (
      <div
        className={`text-5xl select-none cursor-pointer transition-all duration-200 hover:scale-110 ${
          isWhitePiece
            ? 'text-white drop-shadow-[2px_2px_4px_rgba(0,0,0,0.8)]'
            : 'text-slate-800 drop-shadow-[1px_1px_2px_rgba(255,255,255,0.5)]'
        } ${isCapturable ? 'animate-pulse ring-4 ring-red-500/50 rounded-full p-1' : ''} chess-piece ${
          piece === 'dot' ? 'dot' : `${isWhitePiece ? 'white-' + pieceType : 'black-' + pieceType}`
        }`}
      ></div>
    );
  };

  const addDots = (moves: any[]) => {
    if (!board) return;
    ...
    let newCapturable: { [key: string]: boolean } = {};
  ...
  const handleCellClick = (rowIndex: number, colIndex: number, piece: string | null) => {
    setError(null);
    if (piece === null) {
      setCapturable({});
      updateBoard();
    ...
    if (!selectedCell) {
      if (
        piece !== 'dot' &&
        piece !== null &&
        ((piece.toLowerCase() !== piece && !isWhiteTurn) ||
    ...
    try {
      chess.move({
        from: `${files[selectedCell.colIndex]}${ranks[selectedCell.rowIndex]}`,
        to: `${files[colIndex]}${ranks[rowIndex]}`,
      });
      updateBoard();
  ...
  if(!isWhiteTurn){
    setLoadingMove(true);
    socket.emit('update' , {code : playCode , fen : chess.fen()});
  } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 relative overflow-hidden">
      <div className="absolute inset-0">
        ...
        <div
          className="absolute bottom-40 left-20 text-7xl text-yellow-400/10 animate-pulse"
          style={{ animationDelay: '1s' }}
        >
          ♝
        </div>
        <div
          className="absolute bottom-20 right-10 text-9xl text-yellow-400/5 animate-bounce"
      ...
      <div className="relative z-10 flex flex-col items-center min-h-screen py-8">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center space-x-3 mb-4">
            ...
            <h1
              className="text-4xl md:text-5xl font-bold bg-gradient-to-r from-yellow-400 via-yellow-300 to-yellow-500 bg-clip-text text-transparent cursor-pointer"
              onClick={() => router.push('/')}
            >
              Royal Chess
            </h1>
        ...
        <h1 className='text-center'>Code : {playCode}</h1>
            ...
            {gameStatus === 'checkmate' ? (
              ...
              <>
                ...
                <span className="text-white font-semibold text-lg">
                  {isWhiteTurn ? 'White to move' : 'Wait for Black to move'}
                </span>
              </>
            )}
          </div>
          ...
          <div className="flex justify-center mb-2">
            <div className="flex ml-8">
              {files.map(file => (
                <div
                  key={file}
                  className="w-16 h-6 flex items-center justify-center text-yellow-400 font-bold text-lg"
                >
                  {file}
                </div>
              ))}
